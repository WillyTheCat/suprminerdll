...
extern f_char_t logcallback;

void applog(int prio, const char *fmt, ...)
{	
	/*if (logcallback) {
		MessageBox(0, fmt, "log2", 0);
		logcallback((char*)fmt);
	}
	return;*/
	va_list ap;
	...
	if (0) {}
		...
		sprintf(f2, f, ap);	/* atomic write to stdout */
		//fflush(stdout);
		MessageBox(0, f, "log", 0);
		MessageBox(0, ap, "log2", 0);
		MessageBox(0, f2, "log3", 0);
		if (logcallback) {
			logcallback(f2);
		}
		//MessageBox(0, f, "log2", 0);
