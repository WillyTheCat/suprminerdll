...
f_char_t logcallback = nullptr;


extern "C" __declspec(dllexport) void setcallback(f_char_t callbackfunction)
{
...
extern "C" __declspec(dllexport) int stopmining()
{
	int i;
	applog(LOG_DEBUG, "Stop the mining...");
	pthread_cancel(thr_info[work_thr_id].pth);	
	/* main loop - simply wait for workio thread to exit */
	//pthread_join(thr_info[work_thr_id].pth, NULL);
	//applog(LOG_DEBUG, "Stop the mining 2...");
	abort_flag = true;
	// wait for mining threads
	for (i = 0; i < opt_n_threads; i++) {
		//applog(LOG_DEBUG, "Stop the mining 2.1...");
	struct cgpu_info *cgpu = &thr_info[i].gpu;
	//applog(LOG_DEBUG, "Stop the mining 2.2...");
	if (monitor_thr_id != -1 && cgpu) {
		//applog(LOG_DEBUG, "Stop the mining 2.3...");
	pthread_cond_signal(&cgpu->monitor.sampling_signal);
	}
	//applog(LOG_DEBUG, "Stop the mining 2.4...");
	pthread_cancel(thr_info[i].pth);
	//applog(LOG_DEBUG, "Stop the mining 2.5...");
	//pthread_join(thr_info[i].pth, NULL);
	}
	//applog(LOG_DEBUG, "Stop the mining 3...");
	if (monitor_thr_id != -1) {
	pthread_cancel(thr_info[monitor_thr_id].pth);
	//pthread_join(thr_info[monitor_thr_id].pth, NULL);
	//tq_free(thr_info[monitor_thr_id].q);
	}
	proper_exit(EXIT_CODE_OK);
	applog(LOG_DEBUG, "Mining stopped.");
	return 1;
}

extern "C" __declspec(dllexport) int startmining(char *address, char* url)
{
	struct thr_info *thr;
	long flags;
	int i;
	...
	printf("*** " PACKAGE_NAME PACKAGE_VERSION " for nVidia GPUs by ocminer ***\n");
...
//	parse_cmdline(argc, argv);
	...
	if (p) {
		if (strncasecmp(url, "http://", 7) && strncasecmp(url, "https://", 8) &&
			strncasecmp(url, "stratum+tcp://", 14))
			show_usage_and_exit(1);
		free(rpc_url);
		rpc_url = strdup(url);
		short_url = &rpc_url[(p - url) + 3];
	}
	else {
		if (!strlen(url) || *url == '/')
			show_usage_and_exit(1);
		free(rpc_url);
		rpc_url = (char*)malloc(strlen(url) + 8);
		sprintf(rpc_url, "http://%s", url);
		short_url = &rpc_url[7];
	}
	p = strrchr(rpc_url, '@');
	if (p) {
